...
class LvyouPipeline(object):
    ...
    def do_insert(self, cursor, item):
        ...
        insert_sql = """ insert into housingdata(Name,District,BuildYear,BuildingType,PropertyFee,Property_Co,Developers,BuildingsNum,HouseNum,Address,Remarks,FocusNum,AveragePrice,SailNum,Data_HouseCode,Town) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
        cursor.execute(insert_sql,(item['Name'], item['District'], item['BuildYear'],item['BuildingType'], item['PropertyFee'], item['Property_Co'],item['Developers'], item['BuildingsNum'], item['HouseNum'],item['Address'], item['Remarks'], item['FocusNum'],item['AveragePrice'], item['SailNum'], item['Data_HouseCode'],item['Town']))

    def handle_error(self, failure):
        if failure:
            # 打印错误信息
            print(failure)
            # https://vu.qnid.cc/ 这是一个神奇的网站